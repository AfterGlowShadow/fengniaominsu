    ...
    success: function (res) {
      if (res.code) {
        ...
        Post(api.login, {
          ...
          wx.setStorageSync("appid", data2.appid);
          wx.setStorageSync("userinfo", data2.data);
          wx.setStorageSync("userauth", data2.auth)
          wx.setStorageSync("token", data2.token);
...
function checkAuthLogin(type) {
  ...
  // }
  // return true;
  if ((info != "" && info != 0) && (token != "" && token != 0) && (userauth != "" && userauth!=0)){
    return true;
  }else{
    return false;
  }
...
function uploadfileserver(url, that, upload_picture_list, j, data, callback) {
    //上传返回值
  console.log(upload_picture_list);
    const upload_task = wx.uploadFile({
      // 模拟https
      url: module.exports.globalData.apiurl +url, //需要用HTTPS，同时在微信公众平台后台添加服务器地址  
...
module.exports = {
  ...
  Post: Post,
  globalData: {
    apiurl: 'https://api.fengniaorizu.com', appid:'wx4e7b9f2119b57713',appkey:'jiuxiao'},
  that: null,
  api: api,
  lock: 0,//用于其他的锁定wx4e7b9f2119b57713
}
