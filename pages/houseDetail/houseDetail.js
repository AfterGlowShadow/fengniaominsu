...
Page({
  ...
  onLoad: function(options){
    //验证是否实名认证
    if (wx.getStorageSync("userrz") == 0 || wx.getStorageSync("userrz") == "") {
      wx.navigateTo({
    ...
    //获取房屋详情数据
    wxb.Post(wxb.api.house_detail, { houseId: room_id, Uid: wx.getStorageSync('userinfo')}, function (data) {
      that.setData({
        detailList: data,
        price: data.presentPrice,
        yprice: data.orderDeposit
      });
      Toast.clear();
    },'Get');
    var a = 0;
    ...
    //   level: level
    // });
    var starttime = utils.getDay(new Date(), 3);
    var endtime = utils.getDay(new Date(), 3,1);
      that.setData({
        starttime: starttime,
        endtime: endtime
      })
  ...
  Collect:function(e){
    ...
    if(e.currentTarget.dataset.id==0){
      url = wxb.api.Createcollect;
      postdata = {
        HouseID: that.data.detailList.houseId,
        token: token,
        Uid: wx.getStorageSync('userinfo'),
        timeStamp: timestamp
      };
    }else{
      url = wxb.api.Deletecollect;
      postdata = {
        HouseID: that.data.detailList.houseId,
        token: token,
        Uid: wx.getStorageSync('userinfo'),
        timeStamp: timestamp,
        collectID: that.data.detailList.collectID
      };
    }
    wxb.Post(url,  
      postdata , function (data) {
        var houseid = that.data.detailList.houseId;
        that.setData({
          detailList: "",
          price: "",
          yprice: ""
        });
        that.onLoad({ room_id: houseid});
    });
  },
  //跳转到填写订单信息页面
  navigatorToCommitOrder(){
    var that = this;
    var price=that.data.price+that.data.yprice;
    if (!wxb.checkAuthLogin()) {
      wx.navigateTo({
        url: '../login/login',
      });
      return;
    }else if(wx.getStorageSync('userauth')==0){
      Toast({
        message: '请先进行实名认证',
        selector: '#zan-toast-test',
      });
    ...
    } else if (that.data.detailList.db == 1 || that.data.detailList.db==2){
      Toast({
        message: '房屋已经被预定或在保洁中',
        selector: '#zan-toast-test',
      });
    }else{
      wx.navigateTo({
        url: 'commitOrder/commitOrder?room_id=' + that.data.detailList.houseId + '&price=' + price ,
      });
    }
  },
  // 跳转到发票
