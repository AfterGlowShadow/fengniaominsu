...
Page({
  ...
  shangchuanlist: function (url, h_id) {
    ...
    for (var index in imglist) {
      if (imglist[index]['path'] != undefined) {
        newimglist.push(imglist[index]);
      }
    }
    ...
    if(newimglist.length!=0){
      for (var index in newimglist) {
        wxb.uploadfileserver(url, this, newimglist, index, data, function (data) {
          if (data != 1) {
            return 0;
          }
        });
  ...
  submit: function () {
    if (this.data.oneimg == "") {
      ...
      if (newimglist.length >=1) {
        ...
        token = md5(md5(utils.sha1(md5(md5(str)))));
        fyinfo['token'] = token;
        fyinfo['house_id'] = this.data.house_id;
        ...
        //上传方法
        wxb.Post(wxb.api.XiuGaiHoust, fyinfo, function (res) {
          console.log(res);
  ...
  init: function (options,that){
    wxb.Post(wxb.api.HouseInfo, {
      Uid: wx.getStorageSync('userinfo'),
      houseId: options.h_id
    }, function (data) {
      var imglist = that.data.imglist;
      if (data.Img[0]!=""){
        for(var index in data.Img){
          var img={};
      ...
      that.setData({
        ...
        jishi:data.h_room_num,
        jiting:data.h_hall_num,
        fangyuanlx:data.type_id,
        zhujin:data.presentPrice,
        jiren:data.peopleNum,
        title:data.HouseName,
        address:data.location,
        biaoqian:data.label_id,
        chuangleixing:data.bedType,
        mianji:data.Area,
        description:data.MainText,
        onebackground:data.ImageUrl,
        imglist:imglist,
      });
  ...
  deleteimg:function(){
    ...
    var that=this;
    fyinfo['house_id']=this.data.house_id;
    wxb.Post(wxb.api.DeleteImgs, fyinfo, function (data) {
      var oneimglist = that.data.imglist;
        oneimglist.splice(1,oneimglist.length-1);
        that.setData({
          imglist:oneimglist
        })
      // that.onLoad();
